...
/**
 * @ClassName: DjListAdapter
 * @Description:
 * @Author: 常利兵
 * @Date: 2024/4/05 0005 17:24
 **/
class DjListAdapter(val mContext: Context) : AppAdapter<DJXDrama>(mContext) {
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): AppViewHolder {
        return ViewHolder()
    }

    inner class ViewHolder : AppViewHolder(R.layout.item_dj) {
        ...
        override fun onBindView(position: Int) {
            ...
            getItemView().setOnClickListener {
                DjPlayActivity.start(mContext, item.id)
            }
        }
    }
}